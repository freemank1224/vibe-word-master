-- ================================================================
-- Migration: Auto Refresh Leaderboard on Test Completion
-- Date: 2026-02-27
-- ================================================================
-- Purpose: Fix leaderboard not updating in real-time
--
-- Problem: When users complete tests, the leaderboard doesn't update
-- because `record_test_and_sync_stats` only updates daily_stats table
-- but never triggers `calculate_daily_leaderboard` to refresh the
-- leaderboards table.
--
-- Solution: Modify `record_test_and_sync_stats` to automatically
-- trigger leaderboard recalculation for today's date after recording
-- each test. This ensures real-time leaderboard updates.
-- ================================================================

-- ================================================================
-- Function: record_test_and_sync_stats (MODIFIED)
-- ================================================================
-- CHANGE LOG:
-- - Added automatic leaderboard refresh call at the end
-- - Now triggers calculate_daily_leaderboard(v_test_date) before returning
-- ================================================================
...
BEGIN
    ...
    IF v_current_version IS NOT NULL AND p_expected_version IS NOT NULL THEN
        IF v_current_version != p_expected_version THEN
            ...
            -- ✨ NEW: Auto-refresh leaderboard for today after conflict resolution
    ...
    -- ✨ NEW: Auto-refresh leaderboard for today after recording test
    -- This ensures the leaderboard updates in real-time whenever someone completes a test
    PERFORM calculate_daily_leaderboard(v_test_date);
...
-- ================================================================
-- Verification: Check function was created successfully
-- ================================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'record_test_and_sync_stats') THEN
        RAISE NOTICE '✅ Function record_test_and_sync_stats updated successfully';
        RAISE NOTICE '✅ Leaderboard will now auto-refresh after each test completion';
    ELSE
