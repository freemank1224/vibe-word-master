-- ================================================================
-- Migration: Fix version column ambiguity
-- Purpose: Fix ambiguous column reference in record_test_and_sync_stats
-- Date: 2025-02-14
-- Author: System (Phase B Bugfix)
-- ================================================================

-- Drop and recreate function with explicit column references
DROP FUNCTION IF EXISTS record_test_and_sync_stats(p_test_date DATE, p_client_date DATE, p_test_count INTEGER, p_correct_count INTEGER, p_points NUMERIC, p_timezone_offset_hours INTEGER, p_expected_version BIGINT);
...
BEGIN
    ...
    -- Get current version and frozen status (explicitly reference daily_stats)
    SELECT daily_stats.is_frozen, daily_stats.version INTO v_is_frozen, v_current_version
    ...
    IF v_current_version IS NOT NULL AND p_expected_version IS NOT NULL THEN
        IF v_current_version != p_expected_version THEN
            ...
            -- Return merged state (explicitly reference daily_stats in calculation)
            SELECT
                v_test_date,
    ...
    -- Explicitly reference columns to avoid ambiguity
...
-- ================================================================
-- Testing checklist:
-- [ ] Test normal recording (should work now)
-- [ ] Verify version column is correctly updated
