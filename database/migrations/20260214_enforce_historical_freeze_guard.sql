-- ================================================================
-- Migration: Enforce historical stats freeze guard (DB hard lock)
-- Date: 2026-02-14
-- Purpose:
--   1) Keep all dates before Shanghai today frozen
--   2) Reject any write to past/future date at RPC layer
--   3) Preserve version-control behavior
-- ================================================================

-- Keep freeze utility idempotent and available
...
-- Replace latest RPC with hard historical guards
...
BEGIN
    ...
    -- Ensure all days before today are frozen before any write
    ...
    -- Hard guard: only allow writing today's stats
    ...
    -- Keep freeze state fresh after write as well
    PERFORM freeze_previous_days();
...
-- One-time catch-up freeze for existing historical rows
