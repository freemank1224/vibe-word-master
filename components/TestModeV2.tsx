...
const TestModeV2: React.FC<TestModeV2Props> = ({ 
    ...
    const loadResources = async () => {
        ...
        for (let i = 0; i < queue.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async (word) => {
                ...
                try {
                    // 1. Fetch Dictionary Data for phonetic and definition (no audio needed)
                    try {
  ...
  if (isFinished) {
      ...
      const accuracy = queue.length > 0 ? Math.max(0, Math.min(100, Math.round((correctCount / queue.length) * 100))) : 0;
